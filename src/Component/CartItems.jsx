...
export default function () {
  ...
  if (CartItem.length === 0)
    return <h1 className="text-center mt-5" style={{color:"white"}}>Your cart is empty</h1>;
  return (
    <div className=" container-sm d-flex flex-column flex-lg-row mt-4 gap-3  ">
      <div className="d-flex flex-column gap-3" style={{boxShadow:"0 4px 6px rgba(0, 0, 0, 0.774)",borderRadius:"20px"}}>
        {CartItem.map((item) => (
          <Cart  item={item} key={item.id}/>
        ))}
      </div>
      {/* price summary section  */}
      <PriceDetail className="bg-white p-5 d-flex flex-column gap-2  " style={{borderRadius:"20px"}}>
